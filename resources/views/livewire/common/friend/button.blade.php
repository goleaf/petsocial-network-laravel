<div>
    @if($status === 'not_friends')
        <button wire:click="sendRequest" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
            <x-icons.user-plus class="h-4 w-4 mr-1.5" stroke-width="2" /> {{ __('friendships.add_friend') }}
        </button>
    @elseif($status === 'sent_request')
        <div class="relative" x-data="{ open: false }" @click.away="open = false">
            ...
            >
                <x-icons.clock class="h-4 w-4 mr-1.5" stroke-width="2" /> {{ __('friendships.request_sent') }}
            ...
            @if($showDropdown)
                <div class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10">
                    <div class="py-1">
                        ...
                        >
                            <x-icons.x class="h-4 w-4 mr-1.5 text-red-500" stroke-width="2" /> {{ __('friendships.cancel_request') }}
                        </a>
                    </div>
                </div>
            @endif
    ...
    @elseif($status === 'received_request')
        <div class="flex space-x-2">
            ...
            >
                <x-icons.check class="h-4 w-4 mr-1.5" stroke-width="2" /> {{ __('friendships.accept_request') }}
            </button>
            <button 
            ...
            >
                <x-icons.x class="h-4 w-4 mr-1.5" stroke-width="2" /> {{ __('friendships.reject_request') }}
            </button>
        </div>
    @elseif($status === 'friends')
        <div class="relative" x-data="{ open: false }" @click.away="open = false">
            ...
            >
                <x-icons.users class="h-4 w-4 mr-1.5" stroke-width="2" /> {{ __('friendships.friend') }}
                <x-icons.chevron-down class="h-4 w-4 ml-1.5" stroke-width="2" />
            </button>
            @if($showDropdown)
                <div class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10">
                    <div class="py-1">
                        <a 
                            ...
                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 flex items-center"
                        >
                            <x-icons.user-minus class="h-4 w-4 mr-1.5 text-red-500" stroke-width="2" /> {{ __('friendships.remove_friend') }}
                        </a>
                        <a 
                            href="#" 
