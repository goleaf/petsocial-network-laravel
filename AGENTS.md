...
- Language switching behavior is validated across `tests/Feature/LanguageControllerTest.php`, `tests/Http/LanguageControllerHttpTest.php`, `tests/Livewire/LanguageControllerLivewireTest.php`, and `tests/Unit/LanguageControllerTest.php`; refresh those cases when editing `App\\Http\\Controllers\\LanguageController`.
...
- Comprehensive automated coverage for the user settings panel now spans Feature, Unit, Livewire, Filament-simulated, and HTTP suites (`tests/Feature/UserSettingsUpdateTest.php`, `tests/Unit/UserSettingsTest.php`, `tests/Livewire/UserSettingsComponentTest.php`, `tests/Filament/UserSettingsFilamentTest.php`, `tests/Http/UserSettingsRouteTest.php`); update or extend these tests whenever modifying the component.
...
- The shared `layouts.app` template now renders Livewire page component slots alongside Blade sections; ensure new layouts preserve the slot injection so page components keep functioning.
...
- Livewire messaging coverage now lives in `tests/Feature/MessagesReadReceiptsTest.php`, `tests/Livewire/MessagesSendTest.php`, `tests/Unit/MessagesComponentTest.php`, `tests/Filament/MessagesFilamentReadinessTest.php`, and `tests/Http/MessagesRouteTest.php`; keep these suites aligned with changes to `App\Http\Livewire\Messages` and its broadcast events.
...
- Unified search regression coverage now includes dedicated suites in `tests/Livewire/Common/UnifiedSearchLivewireTest.php` and `tests/Http/Common/UnifiedSearchHttpTest.php`; keep these aligned with feature, filament, and unit tests when modifying the component.
